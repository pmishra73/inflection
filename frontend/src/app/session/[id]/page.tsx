...
export default function SessionDetailPage() {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-6 pt-32 text-center">
        <div className="glass-card p-12">
          <div className="w-16 h-16 border-3 border-primary/30 border-t-primary rounded-full animate-spin mx-auto mb-6" style={{ borderWidth: 3 }} />
          <h2 className="text-xl font-semibold text-text-primary mb-2">Analyzing Your Session</h2>
          <p className="text-text-secondary text-sm">
            Claude is generating insights and emotional intelligence report. This takes 30–60 seconds.
          </p>
  ...
  // Radar chart data from top emotions
            ...
            {[
              {
                ...
                value: emotionSummary.dominant_emotion,
                sub: "from voice prosody",
                color: getEmotionColor(emotionSummary.dominant_emotion),
              },
              {
                label: "Emotional Valence",
                value: getValenceLabel(emotionSummary.valence).label,
                sub: `${(emotionSummary.valence * 100).toFixed(0)}% positive`,
                color: getValenceLabel(emotionSummary.valence).color,
              },
              {
                label: "Energy Level",
                value: getArousalLabel(emotionSummary.arousal).label,
                sub: `Emotional arc: ${emotionSummary.emotional_arc}`,
                color: getArousalLabel(emotionSummary.arousal).color,
              },
              {
                label: "Sentiment",
                value: insights?.sentiment_overall
                  ? insights.sentiment_overall.charAt(0).toUpperCase() + insights.sentiment_overall.slice(1)
                  : "—",
                sub: insights?.eq_report?.engagement_level ? `Engagement: ${insights.eq_report.engagement_level}` : "",
                color: insights?.sentiment_overall === "positive" ? "#34d399" : insights?.sentiment_overall === "negative" ? "#f87171" : "#94a3b8",
              },
                ...
                <div className="space-y-2.5">
                  {insights.action_items.map((item: ActionItem, i: number) => (
                    <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-surface-2">
                      <span className={cn("badge border mt-0.5 flex-shrink-0 text-[10px]", PRIORITY_STYLES[item.priority])}>
                        {item.priority}
                      </span>
            ...
            {insights?.eq_report && (
              <Section title="Emotional Intelligence Report" icon={BarChart2}>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-3 rounded-xl bg-surface-2">
                      ...
                      <div className={cn("text-sm font-medium",
                        insights.eq_report.engagement_level === "high" ? "text-accent-green" :
                        insights.eq_report.engagement_level === "low" ? "text-red-400" : "text-accent-yellow"
                      )}>
                  ...
                  {insights.eq_report.emotional_arc && (
                    <div>
                      <div className="text-xs text-text-muted mb-1">Emotional Arc</div>
                      <p className="text-sm text-text-secondary">{insights.eq_report.emotional_arc}</p>
                    </div>
                  )}
                  ...
                  {insights.eq_report.stress_indicators?.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {insights.eq_report.stress_indicators.map((s: string) => (
                          <span key={s} className="badge bg-red-500/10 text-red-400 border border-red-500/20">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {insights.eq_report.positive_moments?.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {insights.eq_report.positive_moments.map((m: string, i: number) => (
                          <li key={i} className="text-sm text-accent-green flex items-start gap-2">
                            <span>✓</span>{m}
                          </li>
                        ))}
                      </ul>
                  ...
                  {insights.incongruence_analysis.examples.map((ex, i) => (
                    <div key={i} className="p-3 rounded-xl bg-surface-2 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="badge bg-accent-yellow/10 text-accent-yellow border border-accent-yellow/20 text-[10px]">
                          {ex.voice_emotion}
                        </span>
                      </div>
            ...
            {emotionSummary && (
              <div className="glass-card p-5">
                <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2">
                  <Brain size={15} className="text-primary-light" />
                  Emotion Summary
                </h3>
                <EmotionDisplay
                  dominantEmotion={emotionSummary.dominant_emotion}
                  topEmotions={emotionSummary.top_emotions || []}
                  valence={emotionSummary.valence}
                  arousal={emotionSummary.arousal}
                />
              </div>
            )}
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="h-1.5 w-16 bg-surface-3 rounded-full overflow-hidden">
                            <div className="h-full bg-primary rounded-full" style={{ width: `${sp.engagement_score * 100}%` }} />
                          </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {sp.dominant_emotions.slice(0, 3).map((e) => (
                          <span key={e} className="text-[10px] px-1.5 py-0.5 rounded-full"
                            style={{ color: getEmotionColor(e), backgroundColor: `${getEmotionColor(e)}15`, border: `1px solid ${getEmotionColor(e)}30` }}>
                            {e}
                          </span>
                        ))}
                      </div>
